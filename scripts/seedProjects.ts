/**
 * Seed Script — Individual Project Card Documents
 *
 * Seeds all project documents from projectData.ts into Sanity CMS.
...
function htmlToPortableText(html: string): any[] {
  ...
  while ((match = tagRegex.exec(cleaned)) !== null) {
    ...
    const rawContent = match[2]
      .replace(/<\/?strong>/gi, '') // strip <strong> for simplicity
      .replace(/<\/?em>/gi, '')
      .replace(/<[^>]*>/g, '')     // strip any remaining inner tags
      .replace(/&amp;/g, '&')
      .replace(/&lt;/g, '<')
      .replace(/&gt;/g, '>')
      .replace(/&#39;/g, "'")
    ...
    if (tag === 'h2') {
      ...
      // Parse <strong>Label:</strong> Rest pattern in list items
      const strongMatch = match[2].match(/<strong>(.*?)<\/strong>\s*(.*)/i)
      let text = rawContent
      if (strongMatch) {
        text = `${strongMatch[1].replace(/<[^>]*>/g, '')} ${strongMatch[2].replace(/<[^>]*>/g, '').trim()}`
      }
      blocks.push({
        _type: 'block',
        _key: key,
...
// ============================================
// All projects from projectData.ts
// ============================================
...
async function main() {
  console.log('☀️  Seeding individual project documents...\n')
  ...
  // First, delete old excluded projects to keep CMS clean
  const excludedSlugs = [
    'gupta-home',
    'singh-residence',
    'city-care-clinic',
  ...
  for (const slug of excludedSlugs) {
    try {
      ...
      if (existing) {
        ...
        console.log(`  🗑️  Deleted excluded project: ${slug}`)
      }
    } catch {
      // ignore
    }
  ...
  for (const project of projects) {
    const docId = `project-${project.slug}`
    const portableTextContent = htmlToPortableText(project.content)
    ...
    // Optional fields
    if (project.inverter) doc.inverter = project.inverter
    if (project.monthlySavings) doc.monthlySavings = project.monthlySavings
    if (project.yearlySavings) doc.yearlySavings = project.yearlySavings
    if (project.shortDescription) doc.shortDescription = project.shortDescription
    if (project.impactBefore) doc.impactBefore = project.impactBefore
    if (project.impactAfter) doc.impactAfter = project.impactAfter
    if (project.tagline) doc.tagline = project.tagline
    if (project.ctaText) doc.ctaText = project.ctaText

    // Order rank for manual sorting
    doc.orderRank = String(project.id).padStart(4, '0')

    await client.createOrReplace(doc)
    console.log(`  ✅ ${project.title} (${project.category}) — ${project.systemSize}`)
  }

  console.log(`\n✅ All ${projects.length} project documents seeded successfully!`)
}
